...
def main():
    ...
    train_dataset = torchaudio.datasets.LIBRISPEECH(config.get("VECTORIZER", "datapath"), url="train-clean-100", download=True)
    test_dataset = torchaudio.datasets.LIBRISPEECH(config.get("VECTORIZER", "datapath"), url="test-clean", download=True)
    ...
    try:
        for epoch in range(start_epoch, config.getint("VECTORIZER", "epochs")):
            # train for one epoch
            now = time.time()
            train(train_loader, model, optimizer, epoch, device, best_acc)
...
def train(train_loader, model, optimizer, epoch, device, best_acc) -> None:
